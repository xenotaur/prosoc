...
| `plaza_crossing` (scenario, name unconfirmed) | `scenarios/perpendicular_traffic`'s Table 3 entry cites "Plaza Crossing" as its related scenario, but no such card exists — the card's own `related_scenarios` substitutes `parallel_traffic`/`intersection_no_gesture` instead | 2026-08-08 | open |
...
| `constitutions/asimov_three_laws` | `scope.exclusions` (`military_combat`, `self-modifying_agents`) claims are contested, not factually wrong — same finding as `asimov_four_laws`'s `## Discussion` "Contested scope claims" bullet (identical exclusions text, byte-for-byte, on both cards). Not a should-fix, a suggestion-level open question the user raised at `asimov_four_laws`'s `APPROVED` review; `asimov_three_laws` was already `APPROVED` before this was noticed, so nothing routed back to it. Consider adding the same "Contested scope claims" Discussion bullet here too, or reverting to a lower state to do so formally — not yet decided | 2026-08-08 | open |
...
| `scenarios/perpendicular_traffic` | "avoid collision with any pedestrian in the flow" — the other `must` item is covered in prose, but this one is never stated as its own bullet | 2026-08-08 |
...
| `scenarios/perpendicular_traffic` | `related_scenarios` lists `intersection_no_gesture`, but `intersection_no_gesture`'s own `related_scenarios` (`intersection_gesture_proceed`/`intersection_gesture_wait`) doesn't reciprocate — a second, previously unnoticed one-way reference distinct from the `crowd_navigation` gap above | 2026-08-08 |
