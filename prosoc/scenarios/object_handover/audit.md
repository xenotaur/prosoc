...
| Related Scenarios | Robot Courier | Card lists `crash_cart` (not `robot_courier`) | Partial — see note below |
| Cited In | [161] | `cited_in: ["161"]`, and Card Summary "Cited In: 161" | Match |

**Note on Related Scenarios divergence:** P&G Table 3 names "Robot Courier" as Object Handover's related scenario, but the card's `related_scenarios` field (and Card Summary) now list `crash_cart` instead. This is not an error: `robot_courier` does not exist as an implemented scenario directory in this repo (confirmed — only `crash_cart` is present under `prosoc/scenarios/`), so it cannot be used as a `related_scenarios` reference (which per schema/convention should point to real scenario ids). The card's own `evaluation_notes` and "Notes for Scenario Designers and Evaluators" section independently discuss *both* Crash Cart and Robot Courier as related, so choosing `crash_cart` — the one that actually exists in the corpus — for the machine-readable field is a reasonable, deliberate choice rather than a fidelity miss. No mismatch flagged; this is provenance context for a human reviewer, not a finding.
...
- `related_scenarios: [crash_cart]` and `cited_in: ["161"]` are both present in `scenario.yml` and schema-valid (arrays of strings).
...
- Related Scenarios — now filled in (`crash_cart`) — previously should-fix, now resolved.
- Cited In — now filled in (`161`) — previously should-fix, now resolved.
