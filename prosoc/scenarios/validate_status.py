...
``prosoc.utils.cards.validate_status`` (see ``WI-CARD-STATUS-TASKS``). This
module forces the ``scenarios`` family and forwards the remaining arguments, so
existing ``from prosoc.scenarios import validate_status`` call sites and tests
(which pass ``--root`` / ``--layout`` / ``--fix``) keep working unchanged.
"""

from __future__ import annotations

from prosoc.utils.cards import validate_status as _generic
